...
public class ExtractProfitPage : Singleton<ExtractProfitPage>
{
    ...
    public void RefreshItem(ExtractsLog data)
    {
        ...
        */
        ...
        JSBridge.inst.ReExtract(data.address, data.amount, data.signature, data.id, "");
    }
    
    public void UpdateItem(GetExtractPurchaseLogSuccess data)
    {
        Game.Inst.extractHistory = data.extract_log;
        list.numItems = 0;
    ...
    private void MakeExtractHistoryItem(ExtractsLog data, GComponent item)
    {
        ...
        item.GetChildByPath("operations").asTextField.visible = true;
        item.GetChildByPath("operations").asTextField.text = "Resubmit";
        if (data.status == 1)
        {
            item.GetChildByPath("operations").asTextField.visible = false;
        }
    }
